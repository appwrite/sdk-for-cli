...
import { Tokens as TokensService } from "@appwrite.io/console";
let tokensClient = null;
const getTokensClient = async () => {
    if (!tokensClient) {
        const projectClient = await sdkForProject();
        tokensClient = new TokensService(projectClient);
    }
    return tokensClient;
};
...
tokens
    ...
    .action(actionRunner(async ({ bucketId, fileId, queries, total }) => await (await getTokensClient()).list(bucketId, fileId, queries, total)));
tokens
    ...
    .action(actionRunner(async ({ bucketId, fileId, expire }) => await (await getTokensClient()).createFileToken(bucketId, fileId, expire)));
tokens
    .command(`get`)
    .description(`Get a token by its unique ID.`)
    .requiredOption(`--token-id <token-id>`, `Token ID.`)
    .option(`--console`, `Get the resource console url`)
    .action(actionRunner(async ({ tokenId }) => await (await getTokensClient()).get(tokenId)));
tokens
    .command(`update`)
    ...
    .action(actionRunner(async ({ tokenId, expire }) => await (await getTokensClient()).update(tokenId, expire)));
tokens
    .command(`delete`)
    .description(`Delete a token by its unique ID.`)
    .requiredOption(`--token-id <token-id>`, `Token ID.`)
    .action(actionRunner(async ({ tokenId }) => await (await getTokensClient()).delete(tokenId)));
