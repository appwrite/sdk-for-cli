...
import { Teams as TeamsService } from "@appwrite.io/console";
let teamsClient = null;
const getTeamsClient = async () => {
    if (!teamsClient) {
        const projectClient = await sdkForProject();
        teamsClient = new TeamsService(projectClient);
    }
    return teamsClient;
};
...
teams
    ...
    .action(actionRunner(async ({ queries, search, total }) => await (await getTeamsClient()).list(queries, search, total)));
teams
    .command(`create`)
    ...
    .action(actionRunner(async ({ teamId, name, roles }) => {
    const rolesArray = roles === true ? [] : roles;
    return await (await getTeamsClient()).create(teamId, name, rolesArray);
}));
teams
    .command(`get`)
    ...
    .option(`--console`, `Get the resource console url`)
    .action(actionRunner(async ({ teamId }) => await (await getTeamsClient()).get(teamId)));
teams
    .command(`update-name`)
    .description(`Update the team's name by its unique ID.`)
    .requiredOption(`--team-id <team-id>`, `Team ID.`)
    .requiredOption(`--name <name>`, `New team name. Max length: 128 chars.`)
    .action(actionRunner(async ({ teamId, name }) => await (await getTeamsClient()).updateName(teamId, name)));
teams
    .command(`delete`)
    .description(`Delete a team using its ID. Only team members with the owner role can delete the team.`)
    .requiredOption(`--team-id <team-id>`, `Team ID.`)
    .action(actionRunner(async ({ teamId }) => await (await getTeamsClient()).delete(teamId)));
teams
    .command(`list-logs`)
    ...
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ teamId, queries, total }) => await (await getTeamsClient()).listLogs(teamId, queries, total)));
teams
    ...
    .option(`--search <search>`, `Search term to filter your list results. Max length: 256 chars.`)
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ teamId, queries, search, total }) => await (await getTeamsClient()).listMemberships(teamId, queries, search, total)));
teams
    ...
    .action(actionRunner(async ({ teamId, roles, email, userId, phone, url, name }) => {
    const rolesArray = roles === true ? [] : roles;
    return await (await getTeamsClient()).createMembership(teamId, rolesArray, email, userId, phone, url, name);
}));
teams
    ...
    .action(actionRunner(async ({ teamId, membershipId }) => await (await getTeamsClient()).getMembership(teamId, membershipId)));
teams
    ...
    .action(actionRunner(async ({ teamId, membershipId, roles }) => {
    const rolesArray = roles === true ? [] : roles;
    return await (await getTeamsClient()).updateMembership(teamId, membershipId, rolesArray);
}));
teams
    ...
    .requiredOption(`--team-id <team-id>`, `Team ID.`)
    .requiredOption(`--membership-id <membership-id>`, `Membership ID.`)
    .action(actionRunner(async ({ teamId, membershipId }) => await (await getTeamsClient()).deleteMembership(teamId, membershipId)));
teams
    ...
    .action(actionRunner(async ({ teamId, membershipId, userId, secret }) => await (await getTeamsClient()).updateMembershipStatus(teamId, membershipId, userId, secret)));
teams
    .command(`get-prefs`)
    ...
    .requiredOption(`--team-id <team-id>`, `Team ID.`)
    .action(actionRunner(async ({ teamId }) => await (await getTeamsClient()).getPrefs(teamId)));
teams
    ...
    .action(actionRunner(async ({ teamId, prefs }) => await (await getTeamsClient()).updatePrefs(teamId, JSON.parse(prefs))));
