...
import { actionRunner, success, log, warn, error, hint, commandDescriptions, drawTable, } from "../parser.js";
import { systemHasCommand, isPortTaken, getAllFiles } from "../utils.js";
import { runtimeNames, systemTools, JwtManager, Queue, } from "../emulation/utils.js";
import { dockerStop, dockerCleanup, dockerStart, dockerBuild, dockerPull, } from "../emulation/docker.js";
const runFunction = async ({ port, functionId, withVariables, reload, userId, } = {}) => {
    ...
    if (withVariables) {
        try {
            const { variables: remoteVariables } = await paginate(async () => (await getFunctionsService()).listVariables(func["$id"]), {}, 100, "variables");
            ...
            warn("Remote variables not fetched. Production environment variables will not be available. Reason: " +
                err.message);
        }
    }
    ...
    try {
        ...
        warn("Dynamic API key not generated. Header x-appwrite-key will not be set. Reason: " +
            err.message);
    }
    const headers = {};
        ...
        try {
            ...
            if (tool.isCompiled || dependencyFile) {
                ...
                const filesToCopy = getAllFiles(functionPath)
                    .map((file) => path.relative(functionPath, file))
