...
migrations
    ...
    .action(actionRunner(async ({ queries, search, total }) => await (await getMigrationsClient()).list(queries, search, total)));
migrations
    ...
    .action(actionRunner(async ({ resources, endpoint, projectId, apiKey }) => await (await getMigrationsClient()).createAppwriteMigration(resources, endpoint, projectId, apiKey)));
migrations
    ...
    .action(actionRunner(async ({ resources, endpoint, projectID, key }) => await (await getMigrationsClient()).getAppwriteReport(resources, endpoint, projectID, key)));
migrations
    .command(`create-csvexport`)
    ...
    .action(actionRunner(async ({ resourceId, filename, columns, queries, delimiter, enclosure, escape, header, notify, }) => await (await getMigrationsClient()).createCSVExport(resourceId, filename, columns, queries, delimiter, enclosure, escape, header, notify)));
migrations
    .command(`create-csvimport`)
    ...
    .action(actionRunner(async ({ bucketId, fileId, resourceId, internalFile }) => await (await getMigrationsClient()).createCSVImport(bucketId, fileId, resourceId, internalFile)));
migrations
    ...
    .action(actionRunner(async ({ resources, serviceAccount }) => await (await getMigrationsClient()).createFirebaseMigration(resources, serviceAccount)));
migrations
    ...
    .requiredOption(`--resources [resources...]`, `List of resources to migrate`)
    .requiredOption(`--service-account <service-account>`, `JSON of the Firebase service account credentials`)
    .action(actionRunner(async ({ resources, serviceAccount }) => await (await getMigrationsClient()).getFirebaseReport(resources, serviceAccount)));
migrations
    .command(`create-nhost-migration`)
    ...
    .action(actionRunner(async ({ resources, subdomain, region, adminSecret, database, username, password, port, }) => await (await getMigrationsClient()).createNHostMigration(resources, subdomain, region, adminSecret, database, username, password, port)));
migrations
    .command(`get-nhost-report`)
    ...
    .requiredOption(`--password <password>`, `Source's Database Password.`)
    .option(`--port <port>`, `Source's Database Port.`, parseInteger)
    .action(actionRunner(async ({ resources, subdomain, region, adminSecret, database, username, password, port, }) => await (await getMigrationsClient()).getNHostReport(resources, subdomain, region, adminSecret, database, username, password, port)));
migrations
    ...
    .requiredOption(`--password <password>`, `Source's Database Password`)
    .option(`--port <port>`, `Source's Database Port`, parseInteger)
    .action(actionRunner(async ({ resources, endpoint, apiKey, databaseHost, username, password, port, }) => await (await getMigrationsClient()).createSupabaseMigration(resources, endpoint, apiKey, databaseHost, username, password, port)));
migrations
    ...
    .requiredOption(`--password <password>`, `Source's Database Password.`)
    .option(`--port <port>`, `Source's Database Port.`, parseInteger)
    .action(actionRunner(async ({ resources, endpoint, apiKey, databaseHost, username, password, port, }) => await (await getMigrationsClient()).getSupabaseReport(resources, endpoint, apiKey, databaseHost, username, password, port)));
migrations
    .command(`get`)
    ...
    .action(actionRunner(async ({ migrationId }) => await (await getMigrationsClient()).get(migrationId)));
migrations
    .command(`retry`)
    .description(`Retry a failed migration. This endpoint allows you to retry a migration that has previously failed.`)
    .requiredOption(`--migration-id <migration-id>`, `Migration unique ID.`)
    .action(actionRunner(async ({ migrationId }) => await (await getMigrationsClient()).retry(migrationId)));
migrations
    .command(`delete`)
    ...
    .action(actionRunner(async ({ migrationId }) => await (await getMigrationsClient()).delete(migrationId)));
