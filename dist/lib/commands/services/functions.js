...
const getFunctionsClient = async () => {
    if (!functionsClient) {
...
functions
    ...
    .action(actionRunner(async ({ queries, search, total }) => await (await getFunctionsClient()).list(queries, search, total)));
functions
    .command(`create`)
...
functions
    ...
    .action(actionRunner(async () => await (await getFunctionsClient()).listRuntimes()));
functions
    .command(`list-specifications`)
    .description(`List allowed function specifications for this instance.`)
    .action(actionRunner(async () => await (await getFunctionsClient()).listSpecifications()));
functions
    ...
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ runtimes, useCases, limit, offset, total }) => await (await getFunctionsClient()).listTemplates(runtimes, useCases, limit, offset, total)));
functions
    ...
    .action(actionRunner(async ({ templateId }) => await (await getFunctionsClient()).getTemplate(templateId)));
functions
    .command(`list-usage`)
    ...
    .action(actionRunner(async ({ range }) => await (await getFunctionsClient()).listUsage(range)));
functions
    .command(`get`)
    .description(`Get a function by its unique ID.`)
    .requiredOption(`--function-id <function-id>`, `Function ID.`)
    .action(actionRunner(async ({ functionId }) => await (await getFunctionsClient()).get(functionId)));
functions
    .command(`update`)
...
functions
    ...
    .requiredOption(`--function-id <function-id>`, `Function ID.`)
    .action(actionRunner(async ({ functionId }) => await (await getFunctionsClient()).delete(functionId)));
functions
    ...
    .action(actionRunner(async ({ functionId, deploymentId }) => await (await getFunctionsClient()).updateFunctionDeployment(functionId, deploymentId)));
functions
    ...
    .option(`--search <search>`, `Search term to filter your list results. Max length: 256 chars.`)
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ functionId, queries, search, total }) => await (await getFunctionsClient()).listDeployments(functionId, queries, search, total)));
functions
...
Use the "command" param to set the entrypoint used to execute your code.`)
    ...
    .action(actionRunner(async ({ functionId, entrypoint, commands, code, activate }) => await (await getFunctionsClient()).createDeployment(functionId, entrypoint, commands, code, activate)));
functions
    ...
    .action(actionRunner(async ({ functionId, deploymentId, buildId }) => await (await getFunctionsClient()).createDuplicateDeployment(functionId, deploymentId, buildId)));
functions
...
Use this endpoint with combination of [listTemplates](https://appwrite.io/docs/products/functions/templates) to find the template details.`)
    ...
    .action(actionRunner(async ({ functionId, repository, owner, rootDirectory, xType, reference, activate, }) => await (await getFunctionsClient()).createTemplateDeployment(functionId, repository, owner, rootDirectory, xType, reference, activate)));
functions
...
This endpoint lets you create deployment from a branch, commit, or a tag.`)
    ...
    .option(`--activate [value]`, `Automatically activate the deployment when it is finished building.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ functionId, xType, reference, activate }) => await (await getFunctionsClient()).createVcsDeployment(functionId, xType, reference, activate)));
functions
    .command(`get-deployment`)
    .description(`Get a function deployment by its unique ID.`)
    .requiredOption(`--function-id <function-id>`, `Function ID.`)
    .requiredOption(`--deployment-id <deployment-id>`, `Deployment ID.`)
    .action(actionRunner(async ({ functionId, deploymentId }) => await (await getFunctionsClient()).getDeployment(functionId, deploymentId)));
functions
    .command(`delete-deployment`)
    .description(`Delete a code deployment by its unique ID.`)
    .requiredOption(`--function-id <function-id>`, `Function ID.`)
    .requiredOption(`--deployment-id <deployment-id>`, `Deployment ID.`)
    .action(actionRunner(async ({ functionId, deploymentId }) => await (await getFunctionsClient()).deleteDeployment(functionId, deploymentId)));
functions
    ...
    .action(actionRunner(async ({ functionId, deploymentId, xType }) => await (await getFunctionsClient()).getDeploymentDownload(functionId, deploymentId, xType)));
functions
    ...
    .requiredOption(`--function-id <function-id>`, `Function ID.`)
    .requiredOption(`--deployment-id <deployment-id>`, `Deployment ID.`)
    .action(actionRunner(async ({ functionId, deploymentId }) => await (await getFunctionsClient()).updateDeploymentStatus(functionId, deploymentId)));
functions
    ...
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ functionId, queries, total }) => await (await getFunctionsClient()).listExecutions(functionId, queries, total)));
functions
    ...
    .action(actionRunner(async ({ functionId, body, xAsync, path, method, headers, scheduledAt, }) => await (await getFunctionsClient()).createExecution(functionId, body, xAsync, path, method, JSON.parse(headers), scheduledAt)));
functions
    ...
    .action(actionRunner(async ({ functionId, executionId }) => await (await getFunctionsClient()).getExecution(functionId, executionId)));
functions
    .command(`delete-execution`)
    .description(`Delete a function execution by its unique ID.`)
    .requiredOption(`--function-id <function-id>`, `Function ID.`)
    .requiredOption(`--execution-id <execution-id>`, `Execution ID.`)
    .action(actionRunner(async ({ functionId, executionId }) => await (await getFunctionsClient()).deleteExecution(functionId, executionId)));
functions
    .command(`get-usage`)
    ...
    .requiredOption(`--function-id <function-id>`, `Function ID.`)
    .option(`--range <range>`, `Date range.`)
    .action(actionRunner(async ({ functionId, range }) => await (await getFunctionsClient()).getUsage(functionId, range)));
functions
    ...
    .action(actionRunner(async ({ functionId }) => await (await getFunctionsClient()).listVariables(functionId)));
functions
    ...
    .action(actionRunner(async ({ functionId, key, value, secret }) => await (await getFunctionsClient()).createVariable(functionId, key, value, secret)));
functions
    .command(`get-variable`)
    .description(`Get a variable by its unique ID.`)
    .requiredOption(`--function-id <function-id>`, `Function unique ID.`)
    .requiredOption(`--variable-id <variable-id>`, `Variable unique ID.`)
    .action(actionRunner(async ({ functionId, variableId }) => await (await getFunctionsClient()).getVariable(functionId, variableId)));
functions
    ...
    .option(`--secret [value]`, `Secret variables can be updated or deleted, but only functions can read them during build and runtime.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ functionId, variableId, key, value, secret }) => await (await getFunctionsClient()).updateVariable(functionId, variableId, key, value, secret)));
functions
    .command(`delete-variable`)
    .description(`Delete a variable by its unique ID.`)
    .requiredOption(`--function-id <function-id>`, `Function unique ID.`)
    .requiredOption(`--variable-id <variable-id>`, `Variable unique ID.`)
    .action(actionRunner(async ({ functionId, variableId }) => await (await getFunctionsClient()).deleteVariable(functionId, variableId)));
