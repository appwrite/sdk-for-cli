...
users
    ...
    .action(actionRunner(async ({ queries, search, total }) => await (await getUsersClient()).list(queries, search, total)));
users
    .command(`create`)
    ...
    .action(actionRunner(async ({ userId, email, phone, password, name }) => await (await getUsersClient()).create(userId, email, phone, password, name)));
users
    ...
    .option(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, email, password, name }) => await (await getUsersClient()).createArgon2User(userId, email, password, name)));
users
    ...
    .option(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, email, password, name }) => await (await getUsersClient()).createBcryptUser(userId, email, password, name)));
users
    ...
    .option(`--search <search>`, `Search term to filter your list results. Max length: 256 chars.`)
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ queries, search, total }) => await (await getUsersClient()).listIdentities(queries, search, total)));
users
    .command(`delete-identity`)
    .description(`Delete an identity by its unique ID.`)
    .requiredOption(`--identity-id <identity-id>`, `Identity ID.`)
    .action(actionRunner(async ({ identityId }) => await (await getUsersClient()).deleteIdentity(identityId)));
users
    ...
    .option(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, email, password, name }) => await (await getUsersClient()).createMD5User(userId, email, password, name)));
users
    ...
    .option(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, email, password, name }) => await (await getUsersClient()).createPHPassUser(userId, email, password, name)));
users
    ...
    .option(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, email, password, passwordSalt, passwordCpu, passwordMemory, passwordParallel, passwordLength, name, }) => await (await getUsersClient()).createScryptUser(userId, email, password, passwordSalt, passwordCpu, passwordMemory, passwordParallel, passwordLength, name)));
users
    ...
    .option(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, email, password, passwordSalt, passwordSaltSeparator, passwordSignerKey, name, }) => await (await getUsersClient()).createScryptModifiedUser(userId, email, password, passwordSalt, passwordSaltSeparator, passwordSignerKey, name)));
users
    ...
    .option(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, email, password, passwordVersion, name }) => await (await getUsersClient()).createSHAUser(userId, email, password, passwordVersion, name)));
users
    .command(`get-usage`)
...
`)
    ...
    .action(actionRunner(async ({ range }) => await (await getUsersClient()).getUsage(range)));
users
    .command(`get`)
    .description(`Get a user by its unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).get(userId)));
users
    .command(`delete`)
    ...
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).delete(userId)));
users
    .command(`update-email`)
    .description(`Update the user email by its unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--email <email>`, `User email.`)
    .action(actionRunner(async ({ userId, email }) => await (await getUsersClient()).updateEmail(userId, email)));
users
    .command(`create-jwt`)
    ...
    .action(actionRunner(async ({ userId, sessionId, duration }) => await (await getUsersClient()).createJWT(userId, sessionId, duration)));
users
...
Labels can be used to grant access to resources. While teams are a way for user's to share access to a resource, labels can be defined by the developer to grant access without an invitation. See the [Permissions docs](https://appwrite.io/docs/permissions) for more info.`)
    ...
    .action(actionRunner(async ({ userId, labels }) => await (await getUsersClient()).updateLabels(userId, labels)));
users
    .command(`list-logs`)
    ...
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ userId, queries, total }) => await (await getUsersClient()).listLogs(userId, queries, total)));
users
    ...
    .option(`--search <search>`, `Search term to filter your list results. Max length: 256 chars.`)
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ userId, queries, search, total }) => await (await getUsersClient()).listMemberships(userId, queries, search, total)));
users
    .command(`update-mfa`)
    .description(`[**DEPRECATED** - This command is deprecated. Please use 'users updateMfa' instead] Enable or disable MFA on a user account.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--mfa <mfa>`, `Enable or disable MFA.`, parseBool)
    .action(actionRunner(async ({ userId, mfa }) => await (await getUsersClient()).updateMfa(userId, mfa)));
users
    .command(`delete-mfa-authenticator`)
    .description(`[**DEPRECATED** - This command is deprecated. Please use 'users deleteMfaAuthenticator' instead] Delete an authenticator app.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--type <type>`, `Type of authenticator.`)
    .action(actionRunner(async ({ userId, xType }) => await (await getUsersClient()).deleteMfaAuthenticator(userId, xType)));
users
    .command(`list-mfa-factors`)
    .description(`[**DEPRECATED** - This command is deprecated. Please use 'users listMfaFactors' instead] List the factors available on the account to be used as a MFA challange.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).listMfaFactors(userId)));
users
    .command(`get-mfa-recovery-codes`)
    .description(`[**DEPRECATED** - This command is deprecated. Please use 'users getMfaRecoveryCodes' instead] Get recovery codes that can be used as backup for MFA flow by User ID. Before getting codes, they must be generated using [createMfaRecoveryCodes](/docs/references/cloud/client-web/account#createMfaRecoveryCodes) method.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).getMfaRecoveryCodes(userId)));
users
    .command(`update-mfa-recovery-codes`)
    .description(`[**DEPRECATED** - This command is deprecated. Please use 'users updateMfaRecoveryCodes' instead] Regenerate recovery codes that can be used as backup for MFA flow by User ID. Before regenerating codes, they must be first generated using [createMfaRecoveryCodes](/docs/references/cloud/client-web/account#createMfaRecoveryCodes) method.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).updateMfaRecoveryCodes(userId)));
users
    .command(`create-mfa-recovery-codes`)
    .description(`[**DEPRECATED** - This command is deprecated. Please use 'users createMfaRecoveryCodes' instead] Generate recovery codes used as backup for MFA flow for User ID. Recovery codes can be used as a MFA verification type in [createMfaChallenge](/docs/references/cloud/client-web/account#createMfaChallenge) method by client SDK.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).createMfaRecoveryCodes(userId)));
users
    .command(`update-name`)
    .description(`Update the user name by its unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--name <name>`, `User name. Max length: 128 chars.`)
    .action(actionRunner(async ({ userId, name }) => await (await getUsersClient()).updateName(userId, name)));
users
    ...
    .action(actionRunner(async ({ userId, password }) => await (await getUsersClient()).updatePassword(userId, password)));
users
    .command(`update-phone`)
    .description(`Update the user phone by its unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--number <number>`, `User phone number.`)
    .action(actionRunner(async ({ userId, number }) => await (await getUsersClient()).updatePhone(userId, number)));
users
    .command(`get-prefs`)
    .description(`Get the user preferences by its unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).getPrefs(userId)));
users
    ...
    .action(actionRunner(async ({ userId, prefs }) => await (await getUsersClient()).updatePrefs(userId, JSON.parse(prefs))));
users
    .command(`list-sessions`)
    .description(`Get the user sessions list by its unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ userId, total }) => await (await getUsersClient()).listSessions(userId, total)));
users
...
If you want to generate a token for a custom authentication flow, use the [POST /users/{userId}/tokens](https://appwrite.io/docs/server/users#createToken) endpoint.`)
    .requiredOption(`--user-id <user-id>`, `User ID. Choose a custom ID or generate a random ID with \`ID.unique()\`. Valid chars are a-z, A-Z, 0-9, period, hyphen, and underscore. Can't start with a special char. Max length is 36 chars.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).createSession(userId)));
users
    .command(`delete-sessions`)
    .description(`Delete all user's sessions by using the user's unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .action(actionRunner(async ({ userId }) => await (await getUsersClient()).deleteSessions(userId)));
users
    .command(`delete-session`)
    .description(`Delete a user sessions by its unique ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--session-id <session-id>`, `Session ID.`)
    .action(actionRunner(async ({ userId, sessionId }) => await (await getUsersClient()).deleteSession(userId, sessionId)));
users
    ...
    .action(actionRunner(async ({ userId, status }) => await (await getUsersClient()).updateStatus(userId, status)));
users
    ...
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ userId, queries, total }) => await (await getUsersClient()).listTargets(userId, queries, total)));
users
    ...
    .action(actionRunner(async ({ userId, targetId, providerType, identifier, providerId, name, }) => await (await getUsersClient()).createTarget(userId, targetId, providerType, identifier, providerId, name)));
users
    .command(`get-target`)
    .description(`Get a user's push notification target by ID.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--target-id <target-id>`, `Target ID.`)
    .action(actionRunner(async ({ userId, targetId }) => await (await getUsersClient()).getTarget(userId, targetId)));
users
    ...
    .option(`--provider-id <provider-id>`, `Provider ID. Message will be sent to this target from the specified provider ID. If no provider ID is set the first setup provider will be used.`)
    .option(`--name <name>`, `Target name. Max length: 128 chars. For example: My Awesome App Galaxy S23.`)
    .action(actionRunner(async ({ userId, targetId, identifier, providerId, name }) => await (await getUsersClient()).updateTarget(userId, targetId, identifier, providerId, name)));
users
    .command(`delete-target`)
    .description(`Delete a messaging target.`)
    .requiredOption(`--user-id <user-id>`, `User ID.`)
    .requiredOption(`--target-id <target-id>`, `Target ID.`)
    .action(actionRunner(async ({ userId, targetId }) => await (await getUsersClient()).deleteTarget(userId, targetId)));
users
...
`)
    ...
    .action(actionRunner(async ({ userId, length, expire }) => await (await getUsersClient()).createToken(userId, length, expire)));
users
    ...
    .action(actionRunner(async ({ userId, emailVerification }) => await (await getUsersClient()).updateEmailVerification(userId, emailVerification)));
users
    ...
    .action(actionRunner(async ({ userId, phoneVerification }) => await (await getUsersClient()).updatePhoneVerification(userId, phoneVerification)));
