...
project
    ...
    .action(actionRunner(async ({ startDate, endDate, period }) => await (await getProjectClient()).getUsage(startDate, endDate, period)));
project
    ...
    .action(actionRunner(async () => await (await getProjectClient()).listVariables()));
project
    ...
    .action(actionRunner(async ({ key, value, secret }) => await (await getProjectClient()).createVariable(key, value, secret)));
project
    .command(`get-variable`)
    .description(`Get a project variable by its unique ID.`)
    .requiredOption(`--variable-id <variable-id>`, `Variable unique ID.`)
    .action(actionRunner(async ({ variableId }) => await (await getProjectClient()).getVariable(variableId)));
project
    ...
    .option(`--secret [value]`, `Secret variables can be updated or deleted, but only projects can read them during build and runtime.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ variableId, key, value, secret }) => await (await getProjectClient()).updateVariable(variableId, key, value, secret)));
project
    .command(`delete-variable`)
    .description(`Delete a project variable by its unique ID. `)
    .requiredOption(`--variable-id <variable-id>`, `Variable unique ID.`)
    .action(actionRunner(async ({ variableId }) => await (await getProjectClient()).deleteVariable(variableId)));
