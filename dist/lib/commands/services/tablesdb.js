...
tablesdb
    ...
    .action(actionRunner(async ({ databaseId, tableId, name, permissions, rowSecurity, enabled, columns, indexes, }) => await (await getTablesDBClient()).createTable(databaseId, tableId, name, permissions, rowSecurity, enabled, columns, indexes)));
tablesdb
    .command(`get-table`)
...
tablesdb
    ...
    .option(`--row-security [value]`, `Enables configuring permissions for individual rows. A user needs one of row or table-level permissions to access a row. [Learn more about permissions](https://appwrite.io/docs/permissions).`, (value) => value === undefined ? true : parseBool(value))
    .option(`--enabled [value]`, `Is table enabled? When set to 'disabled', users cannot access the table but Server SDKs with and API key can still read and write to the table. No data is lost when this is toggled.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, name, permissions, rowSecurity, enabled, }) => await (await getTablesDBClient()).updateTable(databaseId, tableId, name, permissions, rowSecurity, enabled)));
tablesdb
...
`)
    ...
    .option(`--default [value]`, `Default value for column when not provided. Cannot be set when column is required.`, (value) => value === undefined ? true : parseBool(value))
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, array }) => await (await getTablesDBClient()).createBooleanColumn(databaseId, tableId, key, required, xDefault, array)));
tablesdb
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .requiredOption(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`, parseBool)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updateBooleanColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .option(`--default <default>`, `Default value for the column in [ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html) format. Cannot be set when column is required.`)
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, array }) => await (await getTablesDBClient()).createDatetimeColumn(databaseId, tableId, key, required, xDefault, array)));
tablesdb
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .requiredOption(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updateDatetimeColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
...
`)
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .option(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`)
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, array }) => await (await getTablesDBClient()).createEmailColumn(databaseId, tableId, key, required, xDefault, array)));
tablesdb
...
`)
    ...
    .requiredOption(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updateEmailColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
    ...
    .option(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`)
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, elements, required, xDefault, array, }) => await (await getTablesDBClient()).createEnumColumn(databaseId, tableId, key, elements, required, xDefault, array)));
tablesdb
...
`)
    ...
    .requiredOption(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, elements, required, xDefault, newKey, }) => await (await getTablesDBClient()).updateEnumColumn(databaseId, tableId, key, elements, required, xDefault, newKey)));
tablesdb
...
`)
    ...
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, required, min, max, xDefault, array, }) => await (await getTablesDBClient()).createFloatColumn(databaseId, tableId, key, required, min, max, xDefault, array)));
tablesdb
...
`)
    ...
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, min, max, xDefault, newKey, }) => await (await getTablesDBClient()).updateFloatColumn(databaseId, tableId, key, required, min, max, xDefault, newKey)));
tablesdb
...
`)
    ...
    .option(`--min <min>`, `Minimum value`, parseInteger)
    .option(`--max <max>`, `Maximum value`, parseInteger)
    .option(`--default <default>`, `Default value. Cannot be set when column is required.`, parseInteger)
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, required, min, max, xDefault, array, }) => await (await getTablesDBClient()).createIntegerColumn(databaseId, tableId, key, required, min, max, xDefault, array)));
tablesdb
...
`)
    ...
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, min, max, xDefault, newKey, }) => await (await getTablesDBClient()).updateIntegerColumn(databaseId, tableId, key, required, min, max, xDefault, newKey)));
tablesdb
...
`)
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .option(`--default <default>`, `Default value. Cannot be set when column is required.`)
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, array }) => await (await getTablesDBClient()).createIpColumn(databaseId, tableId, key, required, xDefault, array)));
tablesdb
...
`)
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .requiredOption(`--default <default>`, `Default value. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updateIpColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .option(`--default [default...]`, `Default value for column when not provided, two-dimensional array of coordinate pairs, [[longitude, latitude], [longitude, latitude], …], listing the vertices of the line in order. Cannot be set when column is required.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault }) => await (await getTablesDBClient()).createLineColumn(databaseId, tableId, key, required, xDefault)));
tablesdb
    ...
    .option(`--default [default...]`, `Default value for column when not provided, two-dimensional array of coordinate pairs, [[longitude, latitude], [longitude, latitude], …], listing the vertices of the line in order. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updateLineColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .option(`--default [default...]`, `Default value for column when not provided, array of two numbers [longitude, latitude], representing a single coordinate. Cannot be set when column is required.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault }) => await (await getTablesDBClient()).createPointColumn(databaseId, tableId, key, required, xDefault)));
tablesdb
    ...
    .option(`--default [default...]`, `Default value for column when not provided, array of two numbers [longitude, latitude], representing a single coordinate. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updatePointColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
    ...
    .requiredOption(`--key <key>`, `Column Key.`)
    .requiredOption(`--required <required>`, `Is column required?`, parseBool)
    .option(`--default [default...]`, `Default value for column when not provided, three-dimensional array where the outer array holds one or more linear rings, [[[longitude, latitude], …], …], the first ring is the exterior boundary, any additional rings are interior holes, and each ring must start and end with the same coordinate pair. Cannot be set when column is required.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault }) => await (await getTablesDBClient()).createPolygonColumn(databaseId, tableId, key, required, xDefault)));
tablesdb
    ...
    .option(`--default [default...]`, `Default value for column when not provided, three-dimensional array where the outer array holds one or more linear rings, [[[longitude, latitude], …], …], the first ring is the exterior boundary, any additional rings are interior holes, and each ring must start and end with the same coordinate pair. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updatePolygonColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
...
`)
    ...
    .action(actionRunner(async ({ databaseId, tableId, relatedTableId, xType, twoWay, key, twoWayKey, onDelete, }) => await (await getTablesDBClient()).createRelationshipColumn(databaseId, tableId, relatedTableId, xType, twoWay, key, twoWayKey, onDelete)));
tablesdb
...
`)
    ...
    .action(actionRunner(async ({ databaseId, tableId, key, size, required, xDefault, array, encrypt, }) => await (await getTablesDBClient()).createStringColumn(databaseId, tableId, key, size, required, xDefault, array, encrypt)));
tablesdb
...
`)
    ...
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, size, newKey }) => await (await getTablesDBClient()).updateStringColumn(databaseId, tableId, key, required, xDefault, size, newKey)));
tablesdb
...
`)
    ...
    .option(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`)
    .option(`--array [value]`, `Is column an array?`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, array }) => await (await getTablesDBClient()).createUrlColumn(databaseId, tableId, key, required, xDefault, array)));
tablesdb
...
`)
    ...
    .requiredOption(`--default <default>`, `Default value for column when not provided. Cannot be set when column is required.`)
    .option(`--new-key <new-key>`, `New Column Key.`)
    .action(actionRunner(async ({ databaseId, tableId, key, required, xDefault, newKey }) => await (await getTablesDBClient()).updateUrlColumn(databaseId, tableId, key, required, xDefault, newKey)));
tablesdb
    .command(`get-column`)
...
Type can be \`key\`, \`fulltext\`, or \`unique\`.`)
    ...
    .action(actionRunner(async ({ databaseId, tableId, key, xType, columns, orders, lengths }) => await (await getTablesDBClient()).createIndex(databaseId, tableId, key, xType, columns, orders, lengths)));
tablesdb
    .command(`get-index`)
...
tablesdb
    ...
    .action(actionRunner(async ({ databaseId, tableId, rowId, data, permissions, transactionId, }) => await (await getTablesDBClient()).createRow(databaseId, tableId, rowId, JSON.parse(data), permissions, transactionId)));
tablesdb
    .command(`upsert-rows`)
...
tablesdb
    ...
    .option(`--transaction-id <transaction-id>`, `Transaction ID for staging the operation.`)
    .action(actionRunner(async ({ databaseId, tableId, rowId, data, permissions, transactionId, }) => await (await getTablesDBClient()).upsertRow(databaseId, tableId, rowId, JSON.parse(data), permissions, transactionId)));
tablesdb
    .command(`update-row`)
    ...
    .option(`--permissions [permissions...]`, `An array of permissions strings. By default, the current permissions are inherited. [Learn more about permissions](https://appwrite.io/docs/permissions).`)
    .option(`--transaction-id <transaction-id>`, `Transaction ID for staging the operation.`)
    .action(actionRunner(async ({ databaseId, tableId, rowId, data, permissions, transactionId, }) => await (await getTablesDBClient()).updateRow(databaseId, tableId, rowId, JSON.parse(data), permissions, transactionId)));
tablesdb
    .command(`delete-row`)
...
tablesdb
    ...
    .option(`--transaction-id <transaction-id>`, `Transaction ID for staging the operation.`)
    .action(actionRunner(async ({ databaseId, tableId, rowId, column, value, min, transactionId, }) => await (await getTablesDBClient()).decrementRowColumn(databaseId, tableId, rowId, column, value, min, transactionId)));
tablesdb
    ...
    .option(`--transaction-id <transaction-id>`, `Transaction ID for staging the operation.`)
    .action(actionRunner(async ({ databaseId, tableId, rowId, column, value, max, transactionId, }) => await (await getTablesDBClient()).incrementRowColumn(databaseId, tableId, rowId, column, value, max, transactionId)));
tablesdb
    ...
    .option(`--range <range>`, `Date range.`)
    .requiredOption(`--table-id <table-id>`, `Table ID.`)
    .action(actionRunner(async ({ databaseId, range, tableId }) => await (await getTablesDBClient()).getTableUsage(databaseId, range, tableId)));
tablesdb
    .command(`get-usage`)
