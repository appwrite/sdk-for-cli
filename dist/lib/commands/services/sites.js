...
sites
    ...
    .action(actionRunner(async ({ queries, search, total }) => await (await getSitesClient()).list(queries, search, total)));
sites
    .command(`create`)
...
sites
    ...
    .action(actionRunner(async () => await (await getSitesClient()).listFrameworks()));
sites
    .command(`list-specifications`)
    .description(`List allowed site specifications for this instance.`)
    .action(actionRunner(async () => await (await getSitesClient()).listSpecifications()));
sites
    ...
    .action(actionRunner(async ({ frameworks, useCases, limit, offset }) => await (await getSitesClient()).listTemplates(frameworks, useCases, limit, offset)));
sites
    ...
    .action(actionRunner(async ({ templateId }) => await (await getSitesClient()).getTemplate(templateId)));
sites
    .command(`list-usage`)
    ...
    .action(actionRunner(async ({ range }) => await (await getSitesClient()).listUsage(range)));
sites
    .command(`get`)
    .description(`Get a site by its unique ID.`)
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .action(actionRunner(async ({ siteId }) => await (await getSitesClient()).get(siteId)));
sites
    .command(`update`)
...
sites
    ...
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .action(actionRunner(async ({ siteId }) => await (await getSitesClient()).delete(siteId)));
sites
    ...
    .action(actionRunner(async ({ siteId, deploymentId }) => await (await getSitesClient()).updateSiteDeployment(siteId, deploymentId)));
sites
    ...
    .option(`--search <search>`, `Search term to filter your list results. Max length: 256 chars.`)
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ siteId, queries, search, total }) => await (await getSitesClient()).listDeployments(siteId, queries, search, total)));
sites
    ...
    .action(actionRunner(async ({ siteId, installCommand, buildCommand, outputDirectory, code, activate, }) => await (await getSitesClient()).createDeployment(siteId, installCommand, buildCommand, outputDirectory, code, activate)));
sites
    ...
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .requiredOption(`--deployment-id <deployment-id>`, `Deployment ID.`)
    .action(actionRunner(async ({ siteId, deploymentId }) => await (await getSitesClient()).createDuplicateDeployment(siteId, deploymentId)));
sites
...
Use this endpoint with combination of [listTemplates](https://appwrite.io/docs/products/sites/templates) to find the template details.`)
    ...
    .action(actionRunner(async ({ siteId, repository, owner, rootDirectory, xType, reference, activate, }) => await (await getSitesClient()).createTemplateDeployment(siteId, repository, owner, rootDirectory, xType, reference, activate)));
sites
...
This endpoint lets you create deployment from a branch, commit, or a tag.`)
    ...
    .option(`--activate [value]`, `Automatically activate the deployment when it is finished building.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ siteId, xType, reference, activate }) => await (await getSitesClient()).createVcsDeployment(siteId, xType, reference, activate)));
sites
    .command(`get-deployment`)
    .description(`Get a site deployment by its unique ID.`)
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .requiredOption(`--deployment-id <deployment-id>`, `Deployment ID.`)
    .action(actionRunner(async ({ siteId, deploymentId }) => await (await getSitesClient()).getDeployment(siteId, deploymentId)));
sites
    .command(`delete-deployment`)
    .description(`Delete a site deployment by its unique ID.`)
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .requiredOption(`--deployment-id <deployment-id>`, `Deployment ID.`)
    .action(actionRunner(async ({ siteId, deploymentId }) => await (await getSitesClient()).deleteDeployment(siteId, deploymentId)));
sites
    ...
    .action(actionRunner(async ({ siteId, deploymentId, xType }) => await (await getSitesClient()).getDeploymentDownload(siteId, deploymentId, xType)));
sites
    ...
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .requiredOption(`--deployment-id <deployment-id>`, `Deployment ID.`)
    .action(actionRunner(async ({ siteId, deploymentId }) => await (await getSitesClient()).updateDeploymentStatus(siteId, deploymentId)));
sites
    .command(`list-logs`)
    ...
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ siteId, queries, total }) => await (await getSitesClient()).listLogs(siteId, queries, total)));
sites
    .command(`get-log`)
    .description(`Get a site request log by its unique ID.`)
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .requiredOption(`--log-id <log-id>`, `Log ID.`)
    .action(actionRunner(async ({ siteId, logId }) => await (await getSitesClient()).getLog(siteId, logId)));
sites
    .command(`delete-log`)
    .description(`Delete a site log by its unique ID.`)
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .requiredOption(`--log-id <log-id>`, `Log ID.`)
    .action(actionRunner(async ({ siteId, logId }) => await (await getSitesClient()).deleteLog(siteId, logId)));
sites
    .command(`get-usage`)
    ...
    .requiredOption(`--site-id <site-id>`, `Site ID.`)
    .option(`--range <range>`, `Date range.`)
    .action(actionRunner(async ({ siteId, range }) => await (await getSitesClient()).getUsage(siteId, range)));
sites
    .command(`list-variables`)
    .description(`Get a list of all variables of a specific site.`)
    .requiredOption(`--site-id <site-id>`, `Site unique ID.`)
    .action(actionRunner(async ({ siteId }) => await (await getSitesClient()).listVariables(siteId)));
sites
    ...
    .action(actionRunner(async ({ siteId, key, value, secret }) => await (await getSitesClient()).createVariable(siteId, key, value, secret)));
sites
    .command(`get-variable`)
    .description(`Get a variable by its unique ID.`)
    .requiredOption(`--site-id <site-id>`, `Site unique ID.`)
    .requiredOption(`--variable-id <variable-id>`, `Variable unique ID.`)
    .action(actionRunner(async ({ siteId, variableId }) => await (await getSitesClient()).getVariable(siteId, variableId)));
sites
    ...
    .option(`--secret [value]`, `Secret variables can be updated or deleted, but only sites can read them during build and runtime.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ siteId, variableId, key, value, secret }) => await (await getSitesClient()).updateVariable(siteId, variableId, key, value, secret)));
sites
    .command(`delete-variable`)
    .description(`Delete a variable by its unique ID.`)
    .requiredOption(`--site-id <site-id>`, `Site unique ID.`)
    .requiredOption(`--variable-id <variable-id>`, `Variable unique ID.`)
    .action(actionRunner(async ({ siteId, variableId }) => await (await getSitesClient()).deleteVariable(siteId, variableId)));
