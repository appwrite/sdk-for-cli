...
storage
    ...
    .action(actionRunner(async ({ queries, search, total }) => await (await getStorageClient()).listBuckets(queries, search, total)));
storage
...
storage
    ...
    .action(actionRunner(async ({ bucketId }) => await (await getStorageClient()).getBucket(bucketId)));
storage
...
storage
    ...
    .requiredOption(`--bucket-id <bucket-id>`, `Bucket unique ID.`)
    .action(actionRunner(async ({ bucketId }) => await (await getStorageClient()).deleteBucket(bucketId)));
storage
    .command(`list-files`)
    ...
    .option(`--search <search>`, `Search term to filter your list results. Max length: 256 chars.`)
    .option(`--total [value]`, `When set to false, the total count returned will be 0 and will not be calculated.`, (value) => value === undefined ? true : parseBool(value))
    .action(actionRunner(async ({ bucketId, queries, search, total }) => await (await getStorageClient()).listFiles(bucketId, queries, search, total)));
storage
    .command(`create-file`)
...
`)
    ...
    .action(actionRunner(async ({ bucketId, fileId, file, permissions }) => await (await getStorageClient()).createFile(bucketId, fileId, file, permissions)));
storage
    .command(`get-file`)
    ...
    .action(actionRunner(async ({ bucketId, fileId }) => await (await getStorageClient()).getFile(bucketId, fileId)));
storage
    .command(`update-file`)
    ...
    .option(`--permissions [permissions...]`, `An array of permission strings. By default, the current permissions are inherited. [Learn more about permissions](https://appwrite.io/docs/permissions).`)
    .action(actionRunner(async ({ bucketId, fileId, name, permissions }) => await (await getStorageClient()).updateFile(bucketId, fileId, name, permissions)));
storage
    .command(`delete-file`)
    .description(`Delete a file by its unique ID. Only users with write permissions have access to delete this resource.`)
    .requiredOption(`--bucket-id <bucket-id>`, `Storage bucket unique ID. You can create a new storage bucket using the Storage service [server integration](https://appwrite.io/docs/server/storage#createBucket).`)
    .requiredOption(`--file-id <file-id>`, `File ID.`)
    .action(actionRunner(async ({ bucketId, fileId }) => await (await getStorageClient()).deleteFile(bucketId, fileId)));
storage
    ...
    .action(actionRunner(async ({ bucketId, fileId, token }) => await (await getStorageClient()).getFileDownload(bucketId, fileId, token)));
storage
    ...
    .option(`--token <token>`, `File token for accessing this file.`)
    .action(actionRunner(async ({ bucketId, fileId, width, height, gravity, quality, borderWidth, borderColor, borderRadius, opacity, rotation, background, output, token, }) => await (await getStorageClient()).getFilePreview(bucketId, fileId, width, height, gravity, quality, borderWidth, borderColor, borderRadius, opacity, rotation, background, output, token)));
storage
    ...
    .requiredOption(`--file-id <file-id>`, `File ID.`)
    .option(`--token <token>`, `File token for accessing this file.`)
    .action(actionRunner(async ({ bucketId, fileId, token }) => await (await getStorageClient()).getFileView(bucketId, fileId, token)));
storage
    .command(`get-usage`)
...
`)
    ...
    .action(actionRunner(async ({ range }) => await (await getStorageClient()).getUsage(range)));
storage
...
`)
    ...
    .option(`--range <range>`, `Date range.`)
    .action(actionRunner(async ({ bucketId, range }) => await (await getStorageClient()).getBucketUsage(bucketId, range)));
