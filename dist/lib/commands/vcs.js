...
import { Vcs as VcsService, } from "@appwrite.io/console";
let vcsClient = null;
const getVcsClient = async () => {
    if (!vcsClient) {
        const projectClient = await sdkForProject();
        vcsClient = new VcsService(projectClient);
    }
    return vcsClient;
};
...
vcs
    ...
    .action(actionRunner(async ({ installationId, providerRepositoryId, type, providerRootDirectory }) => await (await getVcsClient()).createRepositoryDetection(installationId, providerRepositoryId, type, providerRootDirectory)));
vcs
    ...
    .action(actionRunner(async ({ installationId, type, search, queries }) => await (await getVcsClient()).listRepositories(installationId, type, search, queries)));
vcs
    ...
    .action(actionRunner(async ({ installationId, name, xprivate }) => await (await getVcsClient()).createRepository(installationId, name, xprivate)));
vcs
    ...
    .requiredOption(`--installation-id <installation-id>`, `Installation Id`)
    .requiredOption(`--provider-repository-id <provider-repository-id>`, `Repository Id`)
    .action(actionRunner(async ({ installationId, providerRepositoryId }) => await (await getVcsClient()).getRepository(installationId, providerRepositoryId)));
vcs
    ...
    .requiredOption(`--installation-id <installation-id>`, `Installation Id`)
    .requiredOption(`--provider-repository-id <provider-repository-id>`, `Repository Id`)
    .action(actionRunner(async ({ installationId, providerRepositoryId }) => await (await getVcsClient()).listRepositoryBranches(installationId, providerRepositoryId)));
vcs
    ...
    .action(actionRunner(async ({ installationId, providerRepositoryId, providerRootDirectory, providerReference, }) => await (await getVcsClient()).getRepositoryContents(installationId, providerRepositoryId, providerRootDirectory, providerReference)));
vcs
    ...
    .action(actionRunner(async ({ installationId, repositoryId, providerPullRequestId }) => await (await getVcsClient()).updateExternalDeployments(installationId, repositoryId, providerPullRequestId)));
vcs
    ...
    .action(actionRunner(async ({ queries, search, total }) => await (await getVcsClient()).listInstallations(queries, search, total)));
vcs
    ...
    .requiredOption(`--installation-id <installation-id>`, `Installation Id`)
    .action(actionRunner(async ({ installationId }) => await (await getVcsClient()).getInstallation(installationId)));
vcs
    ...
    .requiredOption(`--installation-id <installation-id>`, `Installation Id`)
    .action(actionRunner(async ({ installationId }) => await (await getVcsClient()).deleteInstallation(installationId)));
