...
import { actionRunner, success, parseBool, commandDescriptions, error, parse, hint, log, drawTable, cliConfig, } from "../parser.js";
import ID from "../id.js";
import { questionsLogin, questionsLogout, questionsListFactors, questionsMFAChallenge, } from "../questions.js";
import { Account } from "@appwrite.io/console";
import ClientLegacy from "../client.js";
const DEFAULT_ENDPOINT = "https://cloud.appwrite.io/v1";
export const loginCommand = async ({ email, password, endpoint, mfa, code, }) => {
    ...
    let account;
    try {
        await legacyClient.call("POST", "/account/sessions/email", {
        ...
        if (err.type === "user_more_factors_required" ||
            ...
            await legacyClient.call("PUT", "/account/mfa/challenges", {
                "content-type": "application/json",
            }, {
...
export const logout = new Command("logout")
    ...
    if (remainingSessions.length > 0 &&
        remainingSessions.filter((session) => session.id === current)
            .length !== 1) {
...
export const client = new Command("client")
    ...
    if (endpoint !== undefined) {
        try {
            ...
            let response = (await clientInstance.call("GET", new URL(endpoint + "/health/version")));
            if (!response.version) {
                throw new Error();
            }
            globalConfig.setCurrentSession(id);
