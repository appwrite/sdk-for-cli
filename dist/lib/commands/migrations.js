...
let migrationsClient = null;
const getMigrationsClient = async () => {
    if (!migrationsClient) {
        const projectClient = await sdkForProject();
        migrationsClient = new Migrations(projectClient);
    }
...
export const migrationsList = async ({ queries, search, total, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).list({ queries, search, total });
    if (parseOutput) {
        parse(response);
    }
    return response;
};
export const migrationsCreateAppwriteMigration = async ({ resources, endpoint, projectId, apiKey, parseOutput = true, }) => {
    const resourcesArray = resources === true ? [] : resources;
    const response = await (await getMigrationsClient()).createAppwriteMigration({
...
export const migrationsGetAppwriteReport = async ({ resources, endpoint, projectID, key, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).getAppwriteReport({
        resources,
        endpoint,
        projectID,
        key,
...
export const migrationsCreateCSVExport = async ({ resourceId, filename, columns, queries, delimiter, enclosure, escape, header, notify, parseOutput = true, }) => {
    const columnsArray = columns === true ? [] : columns;
    const queriesArray = queries === true ? [] : queries;
    const response = await (await getMigrationsClient()).createCSVExport({
        resourceId,
        filename,
...
export const migrationsCreateCSVImport = async ({ bucketId, fileId, resourceId, internalFile, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).createCSVImport({
        bucketId,
        fileId,
        resourceId,
        internalFile,
...
export const migrationsCreateFirebaseMigration = async ({ resources, serviceAccount, parseOutput = true, }) => {
    const resourcesArray = resources === true ? [] : resources;
    const response = await (await getMigrationsClient()).createFirebaseMigration({
        resources: resourcesArray,
        serviceAccount,
    });
    if (parseOutput) {
...
export const migrationsGetFirebaseReport = async ({ resources, serviceAccount, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).getFirebaseReport({
        resources,
        serviceAccount,
    });
    if (parseOutput) {
...
export const migrationsCreateNHostMigration = async ({ resources, subdomain, region, adminSecret, database, username, password, port, parseOutput = true, }) => {
    const resourcesArray = resources === true ? [] : resources;
    const response = await (await getMigrationsClient()).createNHostMigration({
        resources: resourcesArray,
        subdomain,
        region,
        adminSecret,
...
export const migrationsGetNHostReport = async ({ resources, subdomain, region, adminSecret, database, username, password, port, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).getNHostReport({
        resources,
        subdomain,
        region,
        adminSecret,
...
export const migrationsCreateSupabaseMigration = async ({ resources, endpoint, apiKey, databaseHost, username, password, port, parseOutput = true, }) => {
    const resourcesArray = resources === true ? [] : resources;
    const response = await (await getMigrationsClient()).createSupabaseMigration({
        resources: resourcesArray,
        endpoint,
        apiKey,
        databaseHost,
...
export const migrationsGetSupabaseReport = async ({ resources, endpoint, apiKey, databaseHost, username, password, port, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).getSupabaseReport({
        resources,
        endpoint,
        apiKey,
        databaseHost,
...
export const migrationsGet = async ({ migrationId, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).get({ migrationId });
    if (parseOutput) {
        parse(response);
    }
    return response;
};
export const migrationsRetry = async ({ migrationId, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).retry({ migrationId });
    if (parseOutput) {
        parse(response);
    }
    return response;
};
export const migrationsDelete = async ({ migrationId, parseOutput = true, }) => {
    const response = await (await getMigrationsClient()).delete({ migrationId });
    if (parseOutput) {
        parse(response);
    }
    return response;
