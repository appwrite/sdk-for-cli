...
import { getMessagingService, getTeamsService, getProjectsService, getFunctionsService, getSitesService, getDatabasesService, getTablesDBService, getStorageService, } from "../services.js";
import { localConfig } from "../config.js";
import { paginate } from "../paginate.js";
import { questionsPullCollection, questionsPullFunctions, questionsPullFunctionsCode, questionsPullSites, questionsPullSitesCode, questionsPullResources, } from "../questions.js";
import { cliConfig, success, log, warn, error, actionRunner, commandDescriptions, } from "../parser.js";
export const pullResources = async ({ skipDeprecated = false, } = {}) => {
...
const pullFunctions = async ({ code, withVariables, } = {}) => {
    ...
    for (let func of functions) {
        ...
        const downloadBuffer = await client.call("get", new URL(downloadUrl), {}, {}, "arrayBuffer");
...
const pullCollection = async () => {
    ...
    if (databases.length === 0) {
        if (cliConfig.all) {
            databases = (await paginate(async () => (await getDatabasesService()).list(), {}, 100, "databases")).databases.map((database) => database.$id);
        }
        else {
    ...
    for (const databaseId of databases) {
        ...
        const { collections } = await paginate(async () => (await getDatabasesService()).listCollections(databaseId), {}, 100, "collections");
...
const pullTable = async () => {
    ...
    if (databases.length === 0) {
        if (cliConfig.all) {
            databases = (await paginate(async () => (await getTablesDBService()).list(), {}, 100, "databases")).databases.map((database) => database.$id);
        }
        else {
            databases = (await inquirer.prompt(questionsPullCollection)).databases;
        }
    ...
    for (const databaseId of databases) {
        ...
        const { tables } = await paginate(async () => (await getTablesDBService()).listTables(databaseId), {}, 100, "tables");
...
const pullBucket = async () => {
    ...
    const { buckets } = await paginate(async () => (await getStorageService()).listBuckets(), {}, 100, "buckets");
...
const pullTeam = async () => {
    ...
    const { teams } = await paginate(async () => (await getTeamsService()).list(), {}, 100, "teams");
...
const pullMessagingTopic = async () => {
    ...
    const { topics } = await paginate(async () => (await getMessagingService()).listTopics(), {}, 100, "topics");
