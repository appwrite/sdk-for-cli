...
import { actionRunner, commandDescriptions, parseInteger } from "../parser.js";
import { Health as HealthService } from "@appwrite.io/console";
let healthClient = null;
const getHealthClient = async () => {
    if (!healthClient) {
        const projectClient = await sdkForProject();
        healthClient = new HealthService(projectClient);
    }
    return healthClient;
};
...
health
    ...
    .action(actionRunner(async () => await (await getHealthClient()).getAntivirus()));
health
    .command(`get-cache`)
...
health
    ...
    .action(actionRunner(async ({ domain }) => await (await getHealthClient()).getCertificate(domain)));
health
    .command(`get-db`)
...
health
    ...
    .action(actionRunner(async () => await (await getHealthClient()).getPubSub()));
health
    ...
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueBuilds(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueCertificates(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ name, threshold }) => await (await getHealthClient()).getQueueDatabases(name, threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueDeletes(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ name, threshold }) => await (await getHealthClient()).getFailedJobs(name, threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueFunctions(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueLogs(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueMails(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueMessaging(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueMigrations(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueStatsResources(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueUsage(threshold)));
health
    ...
    .option(`--threshold <threshold>`, `Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000.`, parseInteger)
    .action(actionRunner(async ({ threshold }) => await (await getHealthClient()).getQueueWebhooks(threshold)));
health
    .command(`get-storage`)
    .description(`Check the Appwrite storage device is up and connection is successful.`)
    .action(actionRunner(async () => await (await getHealthClient()).getStorage()));
health
    .command(`get-storage-local`)
    .description(`Check the Appwrite local storage device is up and connection is successful.`)
    .action(actionRunner(async () => await (await getHealthClient()).getStorageLocal()));
health
    .command(`get-time`)
