...
function getParamVarName(name: string): string {
  // Convert kebab-case or snake_case to camelCase
  let varName = name.replace(/[-_]([a-z])/g, (_, letter) =>
    letter.toUpperCase(),
  );
...
function needsParser(param: SpecParameter): string | null {
  if (param.type === "boolean") {
    return "parseBool";
  }
  if (param.type === "integer" || param.type === "number") {
...
function getEnumTypesFromParams(params: SpecParameter[]): Set<string> {
...
function generateCommand(
  ...
  serviceName: string,
  clientGetterName: string = `get${capitalizeFirst(serviceName)}Client`,
): string {
  ...
  if (endpoint.parameters) {
    for (const param of endpoint.parameters) {
      if (param.in === "body" && param.schema?.properties) {
        ...
        for (const [propName, propDef] of Object.entries<any>(
  ...
  if (xAppwriteMethods && Array.isArray(xAppwriteMethods)) {
    ...
    if (methodDef) {
      ...
      filteredParams = allParams.filter((param) =>
        allowedParams.has(param.name),
      );
  ...
  for (const param of filteredParams) {
    ...
    const isArray =
      param.type === "array" || param.collectionFormat === "multi";
    const isBool = param.type === "boolean";

    let paramFlag: string;
    if (isArray) {
      paramFlag = `[${paramName}...]`;
  ...
  const paramVarsStr =
    paramVars.length > 0 ? `{ ${paramVars.join(", ")} }` : "";
...
function generateCommandFile(
  serviceName: string,
  endpoints: { [key: string]: SpecEndpoint },
  serviceClassName: string,
  methodToServiceClassMap?: { [methodName: string]: string },
): string {
  // Map method names to their actual service class names
  const actualServiceClassMap: { [methodName: string]: string } =
    methodToServiceClassMap || {};

  // Collect all enum types needed
  const enumTypes = new Set<string>();
  const serviceClassesNeeded = new Set<string>([serviceClassName]);

  for (const endpoint of Object.values(endpoints)) {
    const methodName = endpoint["x-appwrite"].method;
    const actualServiceClass =
      actualServiceClassMap[methodName] || serviceClassName;
  ...
  const clientGetters: { [className: string]: string } = {};
...
async function downloadSpec(url: string): Promise<string> {
...
async function main() {
  ...
  const services: { [key: string]: { [key: string]: SpecEndpoint } } = {};
  const serviceClassNames: { [key: string]: string } = {
    account: "Account",
    users: "Users",
    teams: "Teams",
    databases: "Databases",
  ...
  for (const [pathStr, methods] of Object.entries(spec.paths)) {
    ...
    for (const [method, endpoint] of Object.entries(methods)) {
      if (endpoint && endpoint["x-appwrite"]) {
        ...
        services[serviceName][key] = endpoint as SpecEndpoint;
      }
    }
  }
  ...
  const methodToServiceClassMaps: {
    [serviceName: string]: { [methodName: string]: string };
  } = {
    console: {
      chat: "Assistant",
    },
  ...
  for (const [serviceName, endpoints] of Object.entries(services)) {
    const serviceClassName =
      serviceClassNames[serviceName] || capitalizeFirst(serviceName);
