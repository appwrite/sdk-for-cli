...
const tablesdb = new Command("tablesdb").description(commandDescriptions['tablesdb'] ?? '').configureHelp({
 ...
 * @property {Type} type Database type.
 * @property {boolean} overrideForCli
 * @property {boolean} parseOutput
 * @property {libClient | undefined} sdk
 */
 ...
 */
const tablesdbCreate = async ({databaseId,name,enabled,type,parseOutput = true, overrideForCli = false, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() :
    sdk;
    let apiPath = '/tablesdb';
    let payload = {};
...
tablesdb
    ...
    .description(`Creates an index on the columns listed. Your index should include all the columns you will query in a single request. Attributes can be 'key', 'fulltext', and 'unique'.`)
    .requiredOption(`--database-id <database-id>`, `Database ID.`)
    .requiredOption(`--table-id <table-id>`, `Table ID. You can create a new table using the Database service [server integration](https://appwrite.io/docs/server/tables#tablesCreate).`)
