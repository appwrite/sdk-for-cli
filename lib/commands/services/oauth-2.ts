...
const oauth2AuthorizeCommand = oauth2
  ...
  .option(`--client-id <client-id>`, `OAuth2 client ID. Either a registered app ID or an HTTPS client ID metadata document URL.`)
  ...
  .option(`--audience <audience>`, `Compatibility alias for a single OAuth2 resource indicator URI.`)
  .option(`--request-uri <request-uri>`, `OAuth2 authorization request handle returned by the pushed authorization request endpoint.`)
  .action(
    actionRunner(
      async ({ clientId, redirectUri, responseType, scope, state, nonce, codeChallenge, codeChallengeMethod, prompt, maxAge, authorizationDetails, resource, audience, requestUri }) =>
        parse(await (await getOauth2Client()).authorize(clientId, redirectUri, responseType, scope, state, nonce, codeChallenge, codeChallengeMethod, prompt, maxAge, authorizationDetails, resource, audience, requestUri)),
    ),
  );


const oauth2AuthorizePostCommand = oauth2
  .command(`authorize-post`)
  .description(`Begin the OAuth2 authorization flow. When called without a session, the user is redirected to the consent screen without grant ID. When called with a session, the redirect URL includes param for grant ID. You can pass Accept header of \`application/json\` to receive a JSON response instead of a redirect.`)
  .option(`--client-id <client-id>`, `OAuth2 client ID. Either a registered app ID or an HTTPS client ID metadata document URL.`)
  .option(`--redirect-uri <redirect-uri>`, `Redirect URI where visitor will be redirected after authorization, whether successful or not.`)
  .option(`--response-type <response-type>`, `OAuth2 / OIDC response type. One of \`code\` (Authorization Code Flow), \`id_token\` (Implicit Flow, OIDC login only), or \`code id_token\` (Hybrid Flow).`)
  ...
  .action(
    actionRunner(
      async ({ clientId, redirectUri, responseType, scope, state, nonce, codeChallenge, codeChallengeMethod, prompt, maxAge, authorizationDetails, resource, audience, requestUri }) =>
        parse(await (await getOauth2Client()).authorizePost(clientId, redirectUri, responseType, scope, state, nonce, codeChallenge, codeChallengeMethod, prompt, maxAge, authorizationDetails, resource, audience, requestUri)),
    ),
  );
...
const oauth2CreateDeviceAuthorizationCommand = oauth2
  ...
  .action(
    actionRunner(
      async ({ clientId, scope, authorizationDetails, resource, audience }) =>
        parse(await (await getOauth2Client()).createDeviceAuthorization(clientId, scope, authorizationDetails, resource, audience)),
    ),
  );
...
const oauth2LogoutPostCommand = oauth2
  .command(`logout-post`)
  .description(`OpenID Connect RP-Initiated Logout. Ends the user session and revokes the tokens issued to the app identified by the \`id_token_hint\`, then redirects the user to \`post_logout_redirect_uri\` when it matches a URI registered on the app.`)
  .option(`--id-token-hint <id-token-hint>`, `ID Token previously issued to the app, used as proof of the logout request. Required to end the session; signature and issuer are validated while expiry is ignored.`)
  .option(`--logout-hint <logout-hint>`, `Hint about the user that is logging out. Accepted for OIDC compatibility.`)
  ...
  .action(
    actionRunner(
      async ({ idTokenHint, logoutHint, clientId, postLogoutRedirectUri, state, uiLocales }) =>
        parse(await (await getOauth2Client()).logoutPost(idTokenHint, logoutHint, clientId, postLogoutRedirectUri, state, uiLocales)),
    ),
  );
...
const oauth2CreatePARCommand = oauth2
  ...
  .requiredOption(`--client-id <client-id>`, `OAuth2 client ID. Either a registered app ID or an HTTPS client ID metadata document URL.`)
  ...
  .action(
    actionRunner(
      async ({ clientId, redirectUri, responseType, scope, state, nonce, codeChallenge, codeChallengeMethod, prompt, maxAge, authorizationDetails, resource, audience }) =>
        parse(await (await getOauth2Client()).createPAR(clientId, redirectUri, responseType, scope, state, nonce, codeChallenge, codeChallengeMethod, prompt, maxAge, authorizationDetails, resource, audience)),
    ),
  );
...
const oauth2CreateTokenCommand = oauth2
  ...
  .action(
    actionRunner(
      async ({ grantType, code, refreshToken, deviceCode, clientId, clientSecret, codeVerifier, redirectUri, resource, audience }) =>
        parse(await (await getOauth2Client()).createToken(grantType, code, refreshToken, deviceCode, clientId, clientSecret, codeVerifier, redirectUri, resource, audience)),
    ),
  );
