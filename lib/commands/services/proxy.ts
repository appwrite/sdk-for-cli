  ...
  .action(
    actionRunner(
      async ({ queries, search, total }) =>
        await (await getProxyClient()).listRules(queries, search, total),
    ),
  );

proxy
  .command(`create-apirule`)
  .description(
  ...
  .action(
    actionRunner(
      async ({ domain }) =>
        await (await getProxyClient()).createAPIRule(domain),
    ),
  );
  ...
  .action(
    actionRunner(
      async ({ domain, functionId, branch }) =>
        ...
        ).createFunctionRule(domain, functionId, branch),
    ),
  );
    ...
    actionRunner(
      async ({ domain, url, statusCode, resourceId, resourceType }) =>
        await (
          ...
          statusCode,
          resourceId,
          resourceType as ProxyResourceType,
        ),
    ),
  );
  ...
  .action(
    actionRunner(
      async ({ domain, siteId, branch }) =>
        await (await getProxyClient()).createSiteRule(domain, siteId, branch),
    ),
  );
...
proxy
  ...
  .action(
    actionRunner(
      async ({ ruleId }) => await (await getProxyClient()).getRule(ruleId),
    ),
  );
...
proxy
  ...
  .action(
    actionRunner(
      async ({ ruleId }) => await (await getProxyClient()).deleteRule(ruleId),
    ),
  );
...
proxy
  ...
  .action(
    actionRunner(
      async ({ ruleId }) =>
        await (await getProxyClient()).updateRuleVerification(ruleId),
    ),
  );
