...
const tokensListCommand = tokens
  ...
  .action(
    actionRunner(
      async ({ bucketId, fileId, queries, total, filter, where, sortAsc, sortDesc, cursorAfter, cursorBefore, limit, offset }) =>
        parse(await (await getTokensClient()).list(bucketId, fileId, buildQueries({ queries, filter, where, sortAsc, sortDesc, cursorAfter, cursorBefore, limit, offset }), total)),
    ),
  );
