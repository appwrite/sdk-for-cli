...
const presencesListCommand = presences
  ...
  .action(
    actionRunner(
      async ({ queries, total, ttl, where, sortAsc, sortDesc, cursorAfter, cursorBefore, limit, offset }) =>
        parse(await (await getPresencesClient()).list(buildQueries({ queries, where, sortAsc, sortDesc, cursorAfter, cursorBefore, limit, offset }), total, ttl)),
    ),
  );
...
const presencesUpsertCommand = presences
  ...
  .action(
    actionRunner(
      async ({ presenceId, userId, status, permissions, expiresAt, metadata }) =>
        parse(await (await getPresencesClient()).upsert(presenceId, userId, status, permissions, expiresAt, parseJsonObject(metadata, "--metadata"))),
    ),
  );


const presencesUpdatePresenceCommand = presences
  ...
  .action(
    actionRunner(
      async ({ presenceId, userId, status, expiresAt, metadata, permissions, purge }) =>
