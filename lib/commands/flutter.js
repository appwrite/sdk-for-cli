...
const generate = async (options) => {
    ...
    globalConfig.setProject(answer.project.id, answer.project.name);
    ...
    for (let index in answer.databases) {
        let database = answer.databases[index];
...
function generateDartClass(name, attributes, template) {
    ...
    const properties = attributes.map(attr => {
        let property = `final ${getType(attr.type)}${(!attr.required) ? '?': ''} ${attr.key}`;
        property += ';';
        return property;
    }).join('\n  ');
    ...
    // const toStringFields = attributes.map(attr => {
    //     return `'${attr.key}': ` + '$' + `${attr.key}`;
    // }).join(' ');
