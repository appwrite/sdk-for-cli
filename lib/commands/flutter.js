...
const generate = async (options) => {
    ...
    globalConfig.setProject(projectId);
    ...
    for (let index in databaseIds) {
        ...
        for (let index in collections) {
            ...
            const filename = toSnakeCase(collection.name) + '.dart';
            const className = toUpperCamelCase(collection.name);
...
function generateDartClass(name, attributes, template) {
    const getType = (type, isArray = false) => {
        switch (type) {
            case 'string':
            case 'email':
            case 'url':
            ...
            case 'boolean':
                return isArray ? 'List<bool>' : 'bool';
            case 'integer':
                return isArray ? 'List<int>' : 'int';
            case 'double':
                return isArray ? 'List<double>' : 'double';
        }
    }
    const properties = attributes.map(attr => {
        let property = `final ${getType(attr.type, attr.array)}${(!attr.required) ? '?' : ''} ${attr.key}`;
        property += ';';
        return property;
    }).join('\n  ');
    ...
    const constructorArgs = attributes.map(attr => {
        return `${attr.key}: ${attr.array ? getType(attr.type, attr.array) + '.from(' : ''}map['${attr.key}'] ${attr.array ? ' ?? [])' : ''}`;
    }).join(',\n      ');
