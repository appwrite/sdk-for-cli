...
/**
 * @typedef {Object} AccountCreate2FAChallengeRequestParams
 * @property {Factor} factor Factor used for verification.
 * @property {boolean} parseOutput
 * @property {libClient | undefined} sdk
 */

/**
 * @param {AccountCreate2FAChallengeRequestParams} params
 */
const accountCreate2FAChallenge = async ({ factor, parseOutput = true, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() : sdk;
...
/**
 * @typedef {Object} AccountUpdateChallengeRequestParams
...
/**
 * @param {AccountUpdateChallengeRequestParams} params
 */
const accountUpdateChallenge = async ({ challengeId, otp, parseOutput = true, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() : sdk;
...
/**
 * @typedef {Object} AccountListFactorsRequestParams
 * @property {boolean} parseOutput
 * @property {libClient | undefined} sdk
 */

/**
 * @param {AccountListFactorsRequestParams} params
 */
const accountListFactors = async ({ parseOutput = true, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() : sdk;
...
/**
 * @typedef {Object} AccountAddAuthenticatorRequestParams
 * @property {Type} type Type of authenticator.
 * @property {boolean} parseOutput
 * @property {libClient | undefined} sdk
 */

/**
 * @param {AccountAddAuthenticatorRequestParams} params
 */
const accountAddAuthenticator = async ({ type, parseOutput = true, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() : sdk;
    
    let apiPath = '/account/mfa/{type}'.replace('{type}', type);
    let payload = {};
...
/**
 * @typedef {Object} AccountVerifyAuthenticatorRequestParams
 * @property {Type} type Type of authenticator.
 * @property {string} otp Valid verification token.
 * @property {boolean} parseOutput
 * @property {libClient | undefined} sdk
...
/**
 * @param {AccountVerifyAuthenticatorRequestParams} params
 */
const accountVerifyAuthenticator = async ({ type, otp, parseOutput = true, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() : sdk;
...
/**
 * @typedef {Object} AccountDeleteAuthenticatorRequestParams
 * @property {Type} type Type of authenticator.
 * @property {string} otp Valid verification token.
 * @property {boolean} parseOutput
 * @property {libClient | undefined} sdk
...
/**
 * @param {AccountDeleteAuthenticatorRequestParams} params
 */
const accountDeleteAuthenticator = async ({ type, otp, parseOutput = true, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() : sdk;
 ...
 * @property {boolean} token Include token credentials in the final redirect, useful for server-side integrations, or when cookies are not available.
 ...
 */
const accountCreateOAuth2Session = async ({ provider, success, failure, token, scopes, parseOutput = true, sdk = undefined}) => {
    let client = !sdk ? await sdkForProject() : sdk;
...
account
    .command(`create2FAChallenge`)
    .description(``)
    .requiredOption(`--factor <factor>`, `Factor used for verification.`)
    .action(actionRunner(accountCreate2FAChallenge))

account
    .command(`updateChallenge`)
    .description(``)
    .requiredOption(`--challengeId <challengeId>`, `ID of the challenge.`)
    .requiredOption(`--otp <otp>`, `Valid verification token.`)
    .action(actionRunner(accountUpdateChallenge))

account
    .command(`listFactors`)
    .description(``)
    .action(actionRunner(accountListFactors))

account
    .command(`addAuthenticator`)
    .description(``)
    .requiredOption(`--type <type>`, `Type of authenticator.`)
    .action(actionRunner(accountAddAuthenticator))

account
    .command(`verifyAuthenticator`)
    .description(``)
    .requiredOption(`--type <type>`, `Type of authenticator.`)
    .requiredOption(`--otp <otp>`, `Valid verification token.`)
    .action(actionRunner(accountVerifyAuthenticator))

account
    .command(`deleteAuthenticator`)
    .description(``)
    .requiredOption(`--type <type>`, `Type of authenticator.`)
    .requiredOption(`--otp <otp>`, `Valid verification token.`)
    .action(actionRunner(accountDeleteAuthenticator))
...
account
    ...
    .option(`--token <token>`, `Include token credentials in the final redirect, useful for server-side integrations, or when cookies are not available.`, parseBool)
...
account
    ...
    .description(`Extend session's expiry to increase it's lifespan. Extending a session is useful when session length is short such as 5 minutes.`)
