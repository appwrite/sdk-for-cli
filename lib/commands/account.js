...
const accountCreateChallenge = async ({ provider, parseOutput = true, sdk = undefined}) => {
...
const accountAddAuthenticator = async ({ factor, parseOutput = true, sdk = undefined}) => {
    /* @param {AuthenticatorFactor} factor */

    let client = !sdk ? await sdkForProject() : sdk;
    let apiPath = '/account/mfa/{factor}'.replace('{factor}', factor);
    let payload = {};
    let response = undefined;
    response = await client.call('post', apiPath, {
        'content-type': 'application/json',
...
const accountVerifyAuthenticator = async ({ factor, otp, parseOutput = true, sdk = undefined}) => {
    /* @param {AuthenticatorFactor} factor */
    /* @param {string} otp */
...
const accountDeleteAuthenticator = async ({ provider, otp, parseOutput = true, sdk = undefined}) => {
    /* @param {AuthenticatorProvider} provider */
    /* @param {string} otp */

    let client = !sdk ? await sdkForProject() : sdk;
    let apiPath = '/account/mfa/{provider}'.replace('{provider}', provider);
    let payload = {};
...
account
    ...
    .action(actionRunner(accountCreateChallenge))

account
    .command(`updateChallenge`)
    .description(``)
    .requiredOption(`--challengeId <challengeId>`, `Valid verification token.`)
...
account
    ...
    .description(``)
    .requiredOption(`--factor <factor>`, `Factor.`)
