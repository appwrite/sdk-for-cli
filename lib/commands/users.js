...
const usersDeleteAuthenticator = async ({ userId, provider, otp, parseOutput = true, sdk = undefined}) => {
    /* @param {string} userId */
    /* @param {AuthenticatorProvider} provider */
    /* @param {string} otp */

    let client = !sdk ? await sdkForProject() : sdk;
    let apiPath = '/users/{userId}/mfa/{provider}'.replace('{userId}', userId).replace('{provider}', provider);
    let payload = {};
...
const usersListProviders = async ({ userId, parseOutput = true, sdk = undefined}) => {
    /* @param {string} userId */

    let client = !sdk ? await sdkForProject() : sdk;
    let apiPath = '/users/{userId}/providers'.replace('{userId}', userId);
    let payload = {};
    let response = undefined;
    response = await client.call('get', apiPath, {
        'content-type': 'application/json',
...
users
    ...
    .description(``)
    .requiredOption(`--userId <userId>`, `User ID.`)
    .action(actionRunner(usersListProviders))
