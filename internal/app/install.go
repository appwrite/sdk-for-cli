...
//
// Order matters: npm and Homebrew both leave the binary somewhere
// identifiable, and anything else on PATH is standalone.
...
//
// The naming is contractual across releases, so a new build replaces an
// installed one in place: the asset a user already has and the asset they
// upgrade to have the same name.
...
//
// This is also the constraint on how this CLI is VERSIONED. Every release
// publishes to the same npm package, so a build numbered below what the registry
// already serves would tell every user an upgrade is available and then install
// an older binary over a newer one. The version line has to continue unbroken --
// an rc is `26.0.0-rc.1`, the next major of the same product, not `1.0.0-rc.1`.
// Check what the registry currently serves as `latest` before picking a
// number.
